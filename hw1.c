...
int deepCompare(student_t* students, int numStudents) {
    ...
    for(int i=0; i<numStudents-1; i++)
    {
        ...
        {        
            if((*(students+i)).id != (*(students+j)).id){return 0;}
            if(strcmp( (*(students+i)).name, (*(students+j)).name ) != 0){return 0;}
            if((*(students+i)).gpa != (*(students+j)).gpa){return 0;}
        }
    }
	// ELSE 0
	return 1;
